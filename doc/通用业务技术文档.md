...
| WPS应用扫描 | 识别设备上的WPS Office应用 | app/src/main/java/com/wpspasswordmanager/business/WpsManager.kt |
...
### 3.6 WPS应用识别扫描 (WpsManager)

WpsManager采用四级智能扫描策略识别设备上安装的WPS Office应用，能够覆盖官方标准版、企业定制版及其他特殊版本：

#### 3.6.1 扫描方法

| 方法 | 描述 | 优先级 | 适用场景 |
|------|------|--------|----------|
| 方法1 | 签名验证扫描 | 高（可靠性最高） | 识别使用官方签名的WPS应用 |
| 方法2 | 包名模式匹配扫描 | 中 | 通过包名关键词识别各类WPS版本 |
| 方法3 | 应用标签扫描 | 中 | 通过应用显示名称识别 |
| 方法4 | Intent Filter扫描 | 低（后备） | 通过文档处理能力识别 |

#### 3.6.2 识别特征

**包名关键词：**
- `wps` - WPS品牌关键词
- `moffice` - Mobile Office缩写
- `kingsoft` - 金山软件品牌名
...
扫描结果会自动排除密码管家应用自身（com.wpspasswordmanager），避免将密码管家误识别为WPS Office。

#### 3.6.5 企业版识别支持

该扫描策略不依赖固定包名列表，能够有效识别：
- 官方标准版WPS Office
- 企业定制版WPS（如 cn.wps.moffice.enterprise）
- 特殊定制版本（如内部定制包名）
- 通过应用名称识别（如"金山文档"、"WPS企业版"等）
